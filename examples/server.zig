...
pub fn main(init: std.process.Init) !void {
    ...
    const data_type, const custom_types = try simulatePLCInformationModel(server, gpa);
    defer {
        {
    ...
    {
        var variant: open62541.UA_Variant = undefined;
        const code_ = open62541.UA_Server_readValue(
            server,
            open62541.UA_NODEID_STRING(1, @constCast("OPC_Node_Data")),
            &variant,
        );
        try opc.checkStatusCode(code_);
        std.log.debug(
            "{}",
            .{@as(*types.OPCNodeData, @ptrCast(@alignCast(variant.data)))},
        );
    }
}
...
fn simulatePLCInformationModel(
...
) !struct { opc.UA_DataType.Extended, opc.CustomDataTypes } {
    const ns_idx = 1;
    // Add Dummy Data Type
    ...
    const custom_types: opc.CustomDataTypes = try .init(gpa, 1);
    errdefer custom_types.deinit(gpa);
    custom_types.types[0] = data_type_extended.extern_data_type;
    ...
    custom_data_types.* = .{
        ...
        .typesSize = 1,
        .types = @ptrCast(custom_types.types),
        .cleanup = false,
    };
    ...
    const Val = Slice(types.OPCNodeData);
    var val: Val = .{
        .len = 10,
    ...
    try opc.checkStatusCode(code);
    std.log.debug(
        "{}",
        .{@as(*types.OPCNodeData, @ptrCast(@alignCast(variant.data)))},
    );
    return .{ data_type_extended, custom_types };
}
...
) !opc.UA_DataType.Extended {
    if (@typeInfo(T) != .@"struct") @compileError("Type must be struct");
    const res: opc.UA_DataType.Extended = try .init(
        gpa,
        T,
        "OPC Node Data",
...
fn addVariableType(
    ...
    ns_idx: open62541.UA_UInt16,
    T: type,
    data_type: *opc.UA_DataType.Extern,
    node_identifier_string: []const u8,
    node_display_name: []const u8,
    parent_node_id: open62541.UA_NodeId,
    parent_reference_node_id: open62541.UA_NodeId,
