...
fn browseAddressSpace(
    ...
    var bReq = std.mem.zeroInit(open62541.UA_BrowseRequest, .{});
    bReq.nodesToBrowse = @ptrCast(@alignCast(open62541.UA_new(open62541.UA_DataType_get(open62541.UA_TYPES_BROWSEDESCRIPTION))));
    bReq.nodesToBrowseSize = 1;
    bReq.nodesToBrowse[0].nodeId = node_id;
    bReq.nodesToBrowse[0].resultMask = open62541.UA_BROWSERESULTMASK_ALL;
    ...
    const bresp: opc.UA_BrowseResponse = .init(c_bResp);
    const bresp_header: opc.UA_ResponseHeader = .init(bresp.response_header);
    try opc.checkStatusCode(bresp_header.service_result);
    for (bresp.results[0..bresp.result_size]) |result| {
        for (result.references[0..result.referencesSize]) |ref| {
            const node_class: opc.UANodeClass = @enumFromInt(ref.nodeClass);
            const id_type: opc.UANodeIdType =
